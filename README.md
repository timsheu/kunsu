...
同時開多個軍師／子專案視窗時，`/kunsu-inbox` 得逐一切換視窗手動執行才知道有沒有新訊息，視窗一多容易顧此失彼。`skills/kunsu-dashboard/` 是一個獨立的本機 FastAPI 服務——**軍師沙盤（kunsu dashboard）**，如統帥推演戰局的沙盤，把全域註冊表裡所有軍師與子專案的訊息狀態（待接手交接、已回覆待確認、新回覆、新申請、新上報）彙整成一頁總覽：依軍師分組、子專案巢狀顯示在所屬軍師底下，有進度的軍師預設展開、其餘折疊；每筆交接／訊息可展開看完整 md 內容與最後修改時間。刷新瀏覽器頁面即觸發全新掃描，不跑背景服務。
