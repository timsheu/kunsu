...
### 交接文件
軍師拆解給某子專案的工作項目快照，產出即定案、不做事後修改（唯一例外：done 收尾的生命週期標記）；後續狀態變化一律透過回覆信箱的新檔案表達。
*Avoid:* handoff（口語可用）

### done 收尾
發起方確認接手方回覆無誤後，將交接文件本體 `status` 標記為 `done` 並連同其回覆成對歸檔的具名收尾流程；是「本體不再編輯」原則的唯一例外——發起方對自己文件的生命週期標記，屬授權歸檔、不改內文，單一作者原則不變。僅發起方可執行；接手方勿在回覆檔自標 `done`，自標會使交接從所有掃描面消失、本體卻未歸檔。收尾完成後 `/kunsu-inbox` 與軍師沙盤不再掃描此交接，積壓歸零。
