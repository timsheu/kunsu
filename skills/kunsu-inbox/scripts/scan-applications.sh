#!/usr/bin/env bash
# scan-applications.sh — 掃描軍師 docs/applications/ 中未 commit 的新申請檔案
# 同時執行 tripwire 核對：申請信箱內是否有授權範圍之外的意外變更
#
# 用法：scan-applications.sh <kunsu-root-abs-path>
#
# 輸出（stdout，每行一筆）：
#   NEW_APPLICATION:<相對路徑>    新申請（頂層 .md 檔案，untracked 或 index 新增）
#   TRIPWIRE:<XY> <相對路徑>      意外變更（授權範圍之外的任何狀態變更）
#   TRIPWIRE:<XY> <src> -> <dst>  意外搬移（rename 形式，路徑欄為雙側複合字串）
#
# exit code：
#   0 — 正常完成（含零申請、零 tripwire；docs/applications/ 不存在時同零筆處理）
#   1 — 參數錯誤或非 git repo 根
#   2 — tripwire 觸發（申請信箱有授權範圍外的未 commit 變更）
#
# 分類規則（if/elif 順序即授權邊界，不可調換）：
#   1. docs/applications/archive/* 一律靜默略過 —— 歸檔區由軍師 session 管理，
#      含授權歸檔產生的新增檔案。此分支必須最先評估：bash [[ ]] 的
#      docs/applications/*.md pattern 中 `*` 可跨 `/`，會同時匹配 archive/ 內
#      檔案（與 scan-replies.sh 的單層 replies/ 結構不同，不可直接類比替換）。
#   2. docs/applications/.gitkeep 靜默略過 —— 遷移補建後未 commit 的佔位檔。
#   3. 頂層 <名稱>.md（名稱不含 /）：?? 或 index A ＝新申請；其餘狀態
#      （修改、刪除等）＝tripwire。
#   4. docs/applications/ 下的其他路徑（非預期巢狀、非 .md）＝tripwire。
#   rename（XY 含 R/C，格式 old -> new）：僅「src 為頂層 .md 且 dst 位於
#   archive/」視為授權歸檔（可攜帶內容修改），靜默略過；其餘涉及
#   docs/applications/ 任一側的搬移（含 archive/→頂層反向搬移）＝tripwire。
#
# 路徑處理：
...
if [[ -z "$KUNSU_ROOT" ]]; then
  ...
  echo "用法：scan-applications.sh <kunsu-root-abs-path>" >&2
  exit 1
fi
...
while IFS= read -r line; do
  ...
  if [[ "$path_raw" == *" -> "* ]]; then
    ...
    # 與 docs/applications/ 無關的搬移：略過
    if [[ "$src" != docs/applications/* && "$dst" != docs/applications/* ]]; then
      continue
    fi

    # 授權歸檔豁免：雙側核驗，src 為頂層 .md 且 dst 位於 archive/，
    # 兩條件缺一即走 tripwire（含 archive/→頂層的反向搬移）。
    src_rel="${src#docs/applications/}"
    if [[ "$src" == docs/applications/*.md && "$src_rel" != */* \
          && "$dst" == docs/applications/archive/* ]]; then
      continue
    fi
  ...
  if [[ "$path_part" == docs/applications/archive/* ]]; then
    ...
    continue
  elif [[ "$path_part" == "docs/applications/.gitkeep" ]]; then
    # 遷移補建的佔位檔：靜默略過
    continue
  elif [[ "$path_part" == docs/applications/*.md \
          && "${path_part#docs/applications/}" != */* ]]; then
    # 頂層申請檔：untracked (??) 或 index 新增（X 為 A，涵蓋 A  與 AM）＝新申請
    if [[ "$XY" == "??" ]] || [[ "$X" == "A" ]]; then
...
# 導致 applications/ 目錄本身未被追蹤時新申請無法逐檔偵測）
