...
description: |
  ...
  「回覆交接文件」「產生交接文件」「新增 handoff」「list handoffs」「/handoff」.
allowed-tools:
  - Bash
  - Read
  - Glob
...
- 接手方已完成研究，要回報結論給發起方（`/handoff reply`）
...
> **kunsu 情境約定**：若此 repo 隸屬某軍師（規劃協調中心），`to:` 應使用該軍師登記的**角色代碼**（與 `~/.claude/kunsu-registry.json` 的 `roles` 及軍師 CLAUDE.md 關聯專案表代碼欄字面一致），`/kunsu-inbox` 據此精確比對待接手交接。`/handoff` 作為通用交接原語不強制此約定，僅在 kunsu 語境下成立。
...
1. **取得內容**：這是接手方要交付給發起方的結論，讓對方不必再往返確認就能知道
...
2. **建立回覆檔案**（內文走 stdin）：
   ...
   - 找不到、或找到多筆符合的原交接文件時，腳本會報錯並列出候選，需給更精確的
     片段或完整路徑重新執行。
   - 輸出檔名固定為 `docs/handoffs/replies/{原交接檔名}-reply-{today}.md`；同日
     已存在同名回覆會自動加 `-2`、`-3`…（append-only，不覆寫前次回覆）。

3. **回報**：附上建立的回覆檔案路徑，提醒使用者**不要**回頭編輯交接文件本體。
   **不要**主動 commit。
